...
Class Users extends DBConnection {
	...
	public function save_employee(){
		...
		if($chk > 0){
			...
			if(empty($id)){
				$sql = "INSERT INTO employee_list ($column_str) value ($values_str)";
				if(isset($_POST['proj_id'])){
					$code = $_POST['code'];
					$proj_id = $_POST['proj_id'];
					foreach($proj_id as $id){
					$qry = "INSERT INTO emp_proj_rel (emp_code,proj_id) VALUES ('$code','$id')";
					$this->conn->query($qry);
					}
				}
			}else{
	...
	public function delete_employee(){
		...
		$query = $this->conn->query("SELECT code FROM employee_list where id = $id")->fetch_row();
		$code = $query[0];
		$sql = "DELETE FROM emp_proj_rel where emp_code = '{$code}'";
		$deletion = $this->conn->query($sql);
